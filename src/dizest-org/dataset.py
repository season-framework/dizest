...
class dataset:
    ...
    def regist(self, *args, **package):                
        def wrapper(register):
            ...
            if namespace == "builder":
                raise Exception("dizest: deny using namespace `builder`")
    ...
    def info(self, **info):
        fs = self.__storage__
        data = fs.read.json("dizest.json", dict())

        notallowed = ["data", "path"]
        if len(info) > 0:
            for key in info:
                if key in notallowed: continue
                data[key] = info
            fs.write.json("dizest.json", data)
        
        namespaces = [self.stage["builder"].namespace] + self.stage.namespaces()
        stages = []
